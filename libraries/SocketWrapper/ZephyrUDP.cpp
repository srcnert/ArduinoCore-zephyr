...
uint8_t ZephyrUDP::begin(uint16_t port) {
...
uint8_t ZephyrUDP::beginMulticast(IPAddress ip, uint16_t port) {
	bool ret = begin(port);
	if (ret == false) {
		return false;
	}
	...
	net_if_ipv4_maddr_join(net_if_get_by_index(1),
...
int ZephyrUDP::beginPacket(IPAddress ip, uint16_t port) {
	_send_to_ip = ip;
	_send_to_port = port;
...
int ZephyrUDP::beginPacket(const char *host, uint16_t port) {
	// Resolve address
...
size_t ZephyrUDP::write(uint8_t data) {
	_tx_data.push_back(data);
	return 1;
}

// Write size bytes from buffer into the packet
size_t ZephyrUDP::write(const uint8_t *buffer, size_t size) {
	std::copy(buffer, buffer + size, std::back_inserter(_tx_data));
	return size;
}

int ZephyrUDP::parsePacket() {
	struct sockaddr_in addr;
...
int ZephyrUDP::read(unsigned char *buffer, size_t len) {
	if (_rx_pkt) {
...
int ZephyrUDP::read(char *buffer, size_t len) {
	if (_rx_pkt) {
		return _rx_pkt->read(buffer, len);
	} else {
		return -1;
...
ZephyrUDP::UdpRxPacket::UdpRxPacket(IPAddress const remote_ip, uint16_t const remote_port,
									uint8_t const *p_data, size_t const data_len)
	: _remote_ip(remote_ip), _remote_port(remote_port), _rx_data_len(data_len),
	  _rx_data(p_data, p_data + data_len) {
}

IPAddress ZephyrUDP::UdpRxPacket::remoteIP() const {
	return _remote_ip;
}

uint16_t ZephyrUDP::UdpRxPacket::remotePort() const {
	return _remote_port;
}

size_t ZephyrUDP::UdpRxPacket::totalSize() const {
	return _rx_data_len;
}

int ZephyrUDP::UdpRxPacket::available() {
	return _rx_data.size();
}

int ZephyrUDP::UdpRxPacket::read() {
	uint8_t const data = _rx_data.front();
	_rx_data.pop_front();
	return data;
}

int ZephyrUDP::UdpRxPacket::read(unsigned char *buffer, size_t len) {
	size_t bytes_read = 0;
...
int ZephyrUDP::UdpRxPacket::read(char *buffer, size_t len) {
	return read((unsigned char *)buffer, len);
}

int ZephyrUDP::UdpRxPacket::peek() {
