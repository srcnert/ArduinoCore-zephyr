...
/*
 *  This example reads the WB_IO1 pin and measures the times from high to low,
 *  then light the blue led.
 */
...
void loop() {
	// waits for the pin to go from HIGH to LOW, starts timing, then waits for the pin to go HIGH
	// and stops timing. read the pulsePin time value
	...
	if (duration >= 15000000) {
		// LED turns off when duration exceed 15 seconds
		noTone(ledPin);
	} else if (duration >= 10000000) {
		// LED turns on 5 seconds when duration in 10 ~ 15 seconds
		tone(ledPin, 494, 5000);
	} else if (duration >= 5000000) {
		// LED turns on when seconds 5 seconds and less than 10 seconds
		tone(ledPin, 494);
	}
}
